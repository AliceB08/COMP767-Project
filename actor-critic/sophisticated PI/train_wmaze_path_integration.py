...
PC_SCALE = [argsdict["stdev_place_cells"]]
...
place_cell_ensembles = get_place_cell_ensembles(env_size=argsdict["env_size"], neurons_seed=argsdict["seed"], n_pc=N_PC, \
                                                radial=argsdict["env_watermaze"], radius=argsdict["radius"], pc_scale=PC_SCALE)
    ...
    for e in tqdm(range(start_epoch, argsdict["num_epochs"]+start_epoch)):
        ...
        for X, y in data_generator:
            ...
            if step > argsdict["steps"]:
                break
            step += 1
        tqdm.write(f"EPOCH {e}, training loss : {torch.mean(torch.Tensor(losses))}")
        # tqdm.write(f"training loss : {torch.mean(torch.Tensor(losses))}")
