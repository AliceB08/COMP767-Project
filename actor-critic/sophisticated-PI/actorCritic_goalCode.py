...
from gridCellEncoding import GridCellEncoding

class ActorCritic():
    def __init__(self,env,featureCoding='Place',numCells=None,
        bins=8,
        model_path=None,target_ensemble_path=None,pattern='orthogonal',pc_scale=[0.01]
        ,gamma=1,append_goal=False):
        '''
        Args
            env: environment (watermaze) object where planning is to be performed
            featureCoding: specifies the kind of feature coding -- 'Place' for place cells, 'Tile' for tile coding, 'Grid' for grid coding
        ...
        self.featureCoding = featureCoding
        if self.featureCoding=='Place': 
            self.featureCode = PlaceCellEncoding(self.env,N=numCells,sigma=16)
        elif self.featureCoding=='Tile':
        ...
        elif self.featureCoding=='Grid':
            self.featureCode = GridCellEncoding(env=self.env,model_path=model_path,target_ensemble_path=target_ensemble_path,pattern=pattern,pc_scale=pc_scale)
        if self.featureCoding in ['Place','Tile']: 
            self.featureLength = self.getFeatureEncoding(self.env.position[:,0]).shape[0]
        else:
            self.featureLength = self.featureCode.featureLength
        if self.featureCoding in ['Place','Grid']:
    ...
    def getFeatureEncoding(self,loc=None,ego_motion=None):
        # returns the feature encoding for an (x,y) location in space
        assert loc is not None or ego_motion is not None, "neither loc not ego_motion passed to feature encoder"
        if self.featureCoding in ['Place','Tile']:
            return self.featureCode.getEncoding(loc)
        else:
            return self.featureCode.getEncoding(ego_motion)
    ...
    def get_action(self,loc_features=None,loc=None,ego_motion=None):
        # choose an action using a stochastic policy, with prob proportional to softmax of actor outputs
        if loc_features is None:
            loc_features = self.getFeatureEncoding(loc=loc,ego_motion=ego_motion)
    ...
    def plot_value_function(self):
        ...
        '''
        if self.featureCoding not in ['Place','Tile']:
            warnings.warn("Value function not implemented for Grid/PI coding")
            return
        # create the figure 
        fig = plt.figure()
        ax = fig.gca()
    ...
    def TD_lambda(self,alpha,lamda=0,day=1,episodes=10,verbose=False):
        ...
        for e in range(episodes):
            ...
            curr_loc_encoding = self.getFeatureEncoding(loc=curr_loc,ego_motion=[0.,1.,0.])
            while(not self.env.timeup() and not self.env.atgoal()):
                # select an action from the actor network
                A = self.get_action(loc_features=curr_loc_encoding)
                # move the rat (agent)
                trajectory = self.env.move(A)
                # get next location
                next_loc = self.env.position[:,self.env.t]
                next_loc_encoding = self.getFeatureEncoding(loc=next_loc,ego_motion=trajectory['ego_vel'])
...
if __name__=='__main__':
    ...
    maze.startposition()
    AC = ActorCritic(env=maze,featureCoding=featureCoding,numCells=493,bins=8,
        model_path='exp1/model_epoch_4999.pt',target_ensemble_path='exp1/target_ensembles.pt',pattern='orthogonal',pc_scale=[0.01],
        gamma=0.9,append_goal=append_goal)
