...
def get_scores_and_plot(
    ...
    fig.suptitle('Sorted by {} symmetry score'.format('hexagonal' if sort_by_score_60 else 'orthogonal'),fontsize=16)
    for i in xrange(n_units):
        ...
        if i < n_units:
            ...
            if sort_by_score_60:
                ...
                scorer.plot_sac(sac[index], mask_params=max_90_mask[index], ax=acr, title=title, cmap=cm)
    # Save
    if plot_graphs:
