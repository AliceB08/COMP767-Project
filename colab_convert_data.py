...
def _get_dataset_files(dateset_info, root):
    ...
    return [
        os.path.join(base, template.format(i, num_files - 1)) for i in range(num_files)
    ]
  ...
  """
    ...
    def _make_read_op(self, reader, filename_queue):
        ...
        feature_map = {
            ...
            "ego_vel": tf.FixedLenFeature(
                shape=[self._dataset_info.sequence_length, 3], dtype=tf.float32
            ),
            "target_pos": tf.FixedLenFeature(
                shape=[self._dataset_info.sequence_length, 2], dtype=tf.float32
            ),
            "target_hd": tf.FixedLenFeature(
                shape=[self._dataset_info.sequence_length, 1], dtype=tf.float32
            ),
        }
